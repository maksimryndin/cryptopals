...
use crate::byte_at_a_time_ecb::{SECRET_STRING, BLOCK_SIZE};
use crate::ecb_cbc_oracle::encrypt_aes_ecb;
use set1::convert_hex_to_base64::base64decode;

pub fn ecb_encryption_oracle(random_prefix: &[u8], attacker_input: &[u8], key: [u8; BLOCK_SIZE]) -> Vec<u8> {
...
mod tests {
    use super::*;
    use crate::ecb_cbc_oracle::{random_aes_key, rand_range};
    ...
    fn discover_plaintext_with_random_prefix() {
        ...
        fn bruteforce_block(target_block: &[u8], input: &mut [u8], random_prefix: &[u8], key: [u8; BLOCK_SIZE], prefix_len: usize) -> Option<u8> {
        ...
        for i in 0..no_input_encrypted_len {
            (&mut input[..pad_size + block_size-1]).copy_from_slice(&decrypted[i..i+pad_size + block_size-1]);
            let block_number = i/block_size + prefix_len/block_size; //first blocks contain prefix
            let encrypted = ecb_encryption_oracle(&random_prefix, &input[..pad_size+ block_size - 1 - i % block_size], key);
            let target_block = &encrypted[block_number*block_size..(block_number+1)*block_size];

            if let Some(b) = bruteforce_block(target_block, &mut input, &random_prefix, key, prefix_len) {
                decrypted.push(b);
